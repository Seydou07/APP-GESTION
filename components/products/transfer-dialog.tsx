...
export function TransferStockDialog({ open, onOpenChange, onSuccess, product }: TransferStockDialogProps) {
    ...
    const onSubmit = async (data: any) => {
        ...
        const stockNiveauMagasin = product.stockLevels?.find(sl => sl.warehouseId === 2)?.quantity ?? 0
        if (data.quantite > stockNiveauMagasin) {
            toast.error(`Le stock disponible au magasin est insuffisant (max: ${stockNiveauMagasin})`)
            return
        }
                    ...
                    <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-4 py-4">
                        <div className="bg-muted/40 p-4 rounded-xl space-y-1">
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-2 mt-2 border-t text-xs">
                                <div>
                                    ...
                                    <span className="font-bold text-foreground">{product.stockLevels?.find(sl => sl.warehouseId === 1)?.quantity ?? 0}</span>
                                </div>
                                <div>
                                    <span className="text-muted-foreground block">Stock Magasin (Maison):</span>
                                    <span className="font-bold text-primary">{product.stockLevels?.find(sl => sl.warehouseId === 2)?.quantity ?? 0}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                min={1}
                                max={product.stockLevels?.find(sl => sl.warehouseId === 2)?.quantity ?? 0}
                            />
                            {errors.quantite && <p className="text-xs text-red-500">{errors.quantite.message}</p>}
                        </div>

                        <DialogFooter>
                            <Button type="submit" disabled={loading || (product.stockLevels?.find(sl => sl.warehouseId === 2)?.quantity ?? 0) <= 0} className="w-full">
