...
export function Receipt({ data, showButtons = true }: ReceiptProps) {
    ...
    const itemsTotal = data.items.reduce((sum, item) => sum + (item.priceUnit * Number(item.quantity)), 0)
            ...
            <div className="space-y-6">
                ...
                {data.paymentMethod && (
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold">{methodLabels[data.paymentMethod] || data.paymentMethod}</span>
                    </div>
                )}
                        ...
                        {data.items.map((item, index) => (
                            <TableRow key={index} className="border-none">
                                ...
                                <TableCell className="py-3 text-center font-bold text-xs uppercase">
                                    {typeof item.quantity === 'number' ? `x${item.quantity}` : item.quantity}
                                </TableCell>
                                <TableCell className="py-3 text-right font-bold text-xs">
                                    {(item.priceUnit * Number(item.quantity)).toLocaleString()} F
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                ...
                <div className="border-t-2 border-dashed border-gray-200 pt-4 mt-2 space-y-1">
                    <div className="flex justify-between text-xs text-gray-500">
                        <span>Sous-total</span>
                        <span className="font-mono">{itemsTotal.toLocaleString()} F</span>
                    </div>
                    {data.discount && data.discount > 0 && (
                        <div className="flex justify-between text-xs text-green-600">
                            <span>Remise</span>
                            <span className="font-mono">-{data.discount.toLocaleString()} F</span>
                        </div>
                    )}
