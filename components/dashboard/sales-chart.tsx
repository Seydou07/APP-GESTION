...
export function SalesChart({ data, loading }: SalesChartProps) {
    if (loading || !data) {
        const skeletonHeights = [65, 30, 80, 45, 70, 25, 55]; // Hauteurs fixes pour éviter hydration mismatch
        return (
            <div className="h-[400px] w-full bg-background p-6 rounded-2xl shadow-sm border animate-pulse">
                ...
                <div className="flex items-end justify-between h-[280px] px-4">
                    {[0, 1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="w-10 bg-muted rounded-t-lg" style={{ height: `${skeletonHeights[i]}%` }} />
                    ))}
                </div>
            </div>
        )
