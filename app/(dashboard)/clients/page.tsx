...
import { Search, Users, Plus, Pencil, Trash2, Loader2, Phone, Mail, MapPin, CreditCard, ShoppingCart, AlertTriangle } from "lucide-react"
...
export default function ClientsPage() {
    ...
    const totalCreditLimits = clients.reduce((acc: number, c: any) => acc + (c.creditLimit || 0), 0)

    return (
        <div className="space-y-8 pb-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-black tracking-tight">Gestion des Clients</h2>
                    <p className="text-muted-foreground">Gérez votre fichier clients.</p>
                </div>
                <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                    ...
                    <DialogContent className="sm:max-w-[500px] rounded-3xl p-6">
                        <DialogHeader>
                            <DialogTitle className="text-2xl font-black">Ajouter un Client</DialogTitle>
                        </DialogHeader>
                        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                            <div className="grid gap-2">
                                <Label>Nom complet *</Label>
                                <Input required placeholder="Ex: Jean Dupont" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="rounded-xl" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="grid gap-2">
                                    <Label>Email</Label>
                                    <Input type="email" placeholder="email@exemple.com" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="rounded-xl" />
                                </div>
                            </div>
                            <div className="grid gap-2">
                                <Label>Adresse</Label>
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input type="number" min="0" value={formData.creditLimit} onChange={e => setFormData({ ...formData, creditLimit: Number(e.target.value) })} className="rounded-xl" />
                            </div>
                            <Button type="submit" disabled={submitting} className="w-full rounded-xl font-bold h-12 mt-2">
                                {submitting ? <Loader2 className="animate-spin mr-2" /> : "Créer le client"}
                            </Button>
                        </form>
                    </DialogContent>
                </Dialog>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="rounded-2xl shadow-none border">
                    ...
                    <CardContent>
                        <p className="text-3xl font-black">{totalCreditLimits.toLocaleString()} F</p>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <Table>
                        <TableHeader className="bg-muted/50">
                            <TableRow className="hover:bg-transparent uppercase text-[10px] font-black tracking-widest text-muted-foreground">
                                <TableHead className="pl-8 py-5">Client</TableHead>
                                <TableHead className="py-5">Contact</TableHead>
                                <TableHead className="py-5">Plafond</TableHead>
                                <TableHead className="py-5 text-center">Ventes</TableHead>
                                ...
                                <TableRow key={client.id} className="hover:bg-muted/10 transition-colors group">
                                    <TableCell className="pl-8 py-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <p className="font-bold text-sm tracking-tight">{client.name}</p>
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="py-4 font-mono text-sm font-medium">
                                        {client.creditLimit > 0 ? `${client.creditLimit.toLocaleString()} F` : "-"}
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right pr-8 py-4 space-x-2">
                                        <Button size="icon" variant="ghost" onClick={() => openEditModal(client)} className="rounded-xl text-muted-foreground hover:bg-primary/10 hover:text-primary">
                                            <Pencil className="w-4 h-4" />
                                        </Button>
                                        <Button size="icon" variant="ghost" onClick={() => { setClientToDelete(client.id); setIsConfirmOpen(true) }} className="rounded-xl text-muted-foreground hover:bg-destructive/10 hover:text-destructive">
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                <DialogContent className="sm:max-w-[500px] rounded-3xl p-6">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-black">Modifier le Client</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleUpdate} className="space-y-4 mt-4">
                        ...
                        <div className="grid gap-2">
                            <Label>Plafond de crédit (FCFA)</Label>
                            <Input type="number" min="0" value={editFormData.creditLimit} onChange={e => setEditFormData({ ...editFormData, creditLimit: Number(e.target.value) })} className="rounded-xl" />
                        </div>
                        <Button type="submit" disabled={submitting} className="w-full rounded-xl font-bold h-12 mt-2">
