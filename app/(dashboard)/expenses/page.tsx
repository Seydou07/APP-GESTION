...
export default function ExpensesPage() {
    ...
    const filteredExpenses = expenses.filter((ex: any) => {
        ...
        const descriptionLower = (ex.description ?? "").toLowerCase()
        const categoryLower = (ex.category?.name ?? "Autre").toLowerCase()
    ...
    const totalExpenses = filteredExpenses.reduce((acc, curr: any) => acc + curr.amount, 0)
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card className="rounded-2xl shadow-none border border-border bg-indigo-50/50">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-indigo-600 mt-1">Total période mensuelle</p>
                    </CardContent>
                </Card>
                ...
                <Card className="rounded-2xl shadow-none border border-border bg-red-50/50">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-red-600 mt-1">Somme des éléments affichés</p>
                    </CardContent>
                </Card>
            </div>
                            ...
                            {loading ? (
                                ...
                                <TableRow key={ex.id} className="hover:bg-muted/20">
                                    <TableCell className="pl-8 font-medium">
                                        {format(new Date(ex.expenseDate), "dd MMM yyyy", { locale: fr })}
                                    </TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell>
                                        <div className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-foreground uppercase">
                                            {ex.category?.name ?? "Autre"}
                                        </div>
                                    </TableCell>
                                    <TableCell className="text-right font-bold text-destructive">
                                        - {ex.amount.toLocaleString()} F
                                    </TableCell>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="p-5 bg-muted/30 rounded-2xl border space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-bold text-lg">{selectedExpense.description}</p>
                                    </div>
                                    <div>
                                        ...
                                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-muted text-foreground mt-1">
                                            {selectedExpense.category?.name ?? "Autre"}
                                        </span>
                                    </div>
                                    <div>
                                        <p className="text-xs text-muted-foreground">Date</p>
                                        <p className="font-medium">{format(new Date(selectedExpense.expenseDate), "dd MMMM yyyy", { locale: fr })}</p>
                                    </div>
                                </div>
                                ...
                                <div className="p-5 bg-red-50/50 rounded-2xl border border-red-100 flex flex-col justify-center items-center text-center space-y-2">
                                    ...
                                    <p className="font-black text-4xl text-red-600">{selectedExpense.amount.toLocaleString()} F</p>
                                </div>
                            </div>
