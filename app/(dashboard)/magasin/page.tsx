...
export default function MagasinPage() {
    ...
    const filteredProducts = products
        .filter((p: any) => {
            ...
            const categoryNameLower = (p.category?.name ?? "").toLowerCase()
            const codeLower = (p.code ?? "").toLowerCase()
            
            return nameLower.includes(searchLower) || categoryNameLower.includes(searchLower) || codeLower.includes(searchLower)
        })
        .sort((a: any, b: any) => {
            if (sortOrder === "asc") return (a.name ?? "").localeCompare((b.name ?? ""), "fr", { sensitivity: "base" })
            if (sortOrder === "desc") return (b.name ?? "").localeCompare((a.name ?? ""), "fr", { sensitivity: "base" })
            return 0
        })

    // Stats
    const totalArticlesMagasin = products.reduce((acc: number, p: any) => acc + ((p.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) || 0), 0)
    const totalValeurMagasin = products.reduce((acc: number, p: any) => acc + (p.salePrice * ((p.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) || 0)), 0)
    const produitsEnMagasin = products.filter((p: any) => ((p.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) || 0) > 0)
    const produitsVides = products.filter((p: any) => ((p.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) || 0) === 0)
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow key={product.id}>
                                    ...
                                    <TableCell>{product.salePrice.toLocaleString()} F</TableCell>
                                    <TableCell>
                                        <span className={cn(
                                            "px-3 py-1 rounded-full text-xs font-bold",
                                            (product.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) === 0 ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-600"
                                        )}>
                                            {product.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0}
                                        </span>
                                    </TableCell>
                                    <TableCell>
                                        <span className={cn(
                                            "px-3 py-1 rounded-full text-xs font-bold",
                                            (product.stockLevels?.find((sl: any) => sl.warehouseId === 1)?.quantity ?? 0) <= product.stockMin ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"
                                        )}>
                                            {product.stockLevels?.find((sl: any) => sl.warehouseId === 1)?.quantity ?? 0}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-right pr-6">
                                        <div className="flex justify-end">
                                            <Button
                                                ...
                                                disabled={(product.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) <= 0}
                                                onClick={() => {
                                                ...
                                                className={cn(
                                                    ...
                                                    (product.stockLevels?.find((sl: any) => sl.warehouseId === 2)?.quantity ?? 0) <= 0 && "opacity-40 border-muted text-muted-foreground"
                                                )}
                                            >
