...
export default function DebtsPage() {
    ...
    const fetchMonthlyNewDebts = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const total = data.reduce((acc: number, curr: any) => acc + curr.amountInitial, 0)
    ...
    const handlePrintReceipt = (debt: any, paymentAmount: number, newBalance: number) => {
        setReceiptData({
            ...
            client: debt.client?.name ?? debt.notes,
            ...
            total: paymentAmount,
            extraInfo: [
                { label: "Ancien Reste", value: `${(debt.amountInitial - debt.amountDue).toLocaleString()} F` },
                { label: "Nouveau Reste", value: `${newBalance.toLocaleString()} F` },
                { label: "Téléphone", value: debt.client?.phone ?? "" }
            ],
    ...
    const handlePayment = async (e: React.FormEvent) => {
        ...
        const currentPaid = selectedDebt.amountDue
        const total = selectedDebt.amountInitial
    ...
    const filteredDebts = debts.filter((d: any) => {
        ...
        return clientName.includes(searchLower) || notes.includes(searchLower) || clientPhone.includes(searchTerm)
    })

    const displayedDebts = filteredDebts.slice((page - 1) * pageSize, page * pageSize)

    const totalDettes = filteredDebts.reduce((acc, d: any) => acc + (d.amountInitial - d.amountDue), 0)
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "EN_COURS":
                return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-700"><AlertCircle className="w-3 h-3" /> En Cours</span>
            case "ECHUE":
                return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-orange-100 text-orange-700"><Clock className="w-3 h-3" /> Échue</span>
            case "SOLDEE":
                return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700"><CheckCircle2 className="w-3 h-3" /> Soldée</span>
            default:
                return null
        }
    }
                                ...
                                <TableRow key={d.id} className="hover:bg-muted/20">
                                    <TableCell className="pl-6 font-medium">
                                        <div className="flex flex-col">
                                            <span className="font-bold">{d.client?.name ?? d.notes}</span>
                                            <span className="text-xs text-muted-foreground">{d.client?.phone ?? ""}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell>{format(new Date(d.createdAt), "dd/MM/yyyy")}</TableCell>
                                    <TableCell className="text-right font-mono">{d.amountInitial.toLocaleString()} F</TableCell>
                                    <TableCell className="text-right font-mono text-emerald-600">{d.amountDue.toLocaleString()} F</TableCell>
                                    <TableCell className="text-right font-mono font-bold text-indigo-600">{(d.amountInitial - d.amountDue).toLocaleString()} F</TableCell>
                                    ...
                                    <TableCell className="text-center">
                                        <Button
                                            ...
                                            onClick={() => {
                                                setSelectedDebt(d)
                                                setPaymentAmount((d.amountInitial - d.amountDue).toString())
                                                setIsPayModalOpen(true)
                                            }}
                                            disabled={d.status === "SOLDEE"}
                                            className={cn(
            ...
            <Dialog open={isPayModalOpen} onOpenChange={setIsPayModalOpen}>
                <DialogContent className="sm:max-w-[425px] rounded-3xl p-6">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Règlement pour la dette de <strong>{selectedDebt?.client?.name ?? selectedDebt?.notes}</strong>
                        </DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handlePayment} className="space-y-4 mt-4">
                        <div className="p-4 bg-muted/50 rounded-xl space-y-1">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-bold">{selectedDebt?.amountInitial.toLocaleString()} F</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span>Déjà payé:</span>
                                <span className="font-bold text-emerald-600">{selectedDebt?.amountDue.toLocaleString()} F</span>
                            </div>
                            <div className="flex justify-between text-sm pt-2 border-t mt-2">
                                <span>Reste à payer:</span>
                                <span className="font-bold text-indigo-600">{(selectedDebt?.amountInitial - selectedDebt?.amountDue).toLocaleString()} F</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                type="number"
                                required
                                max={selectedDebt ? selectedDebt.amountInitial - selectedDebt.amountDue : 0}
                                value={paymentAmount}
                                ...
                                <div className="p-6 bg-muted/30 rounded-2xl border space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            <p className="font-bold text-lg">{selectedDebt.client?.name ?? selectedDebt.notes}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-muted-foreground">Téléphone</p>
                                            <p className="font-medium">{selectedDebt.client?.phone ?? ""}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-muted-foreground">Date Création</p>
                                            <p className="font-medium">{format(new Date(selectedDebt.createdAt), "dd MMMM yyyy", { locale: fr })}</p>
                                        </div>
                                        <div>
                                ...
                                <div className="p-6 bg-indigo-50/50 rounded-2xl border border-indigo-100 space-y-4">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center pb-2 border-b border-indigo-100">
                                            ...
                                            <p className="font-black text-xl text-indigo-900">{selectedDebt.amountInitial.toLocaleString()} F</p>
                                        </div>
                                        <div className="flex justify-between items-center pb-2 border-b border-indigo-100">
                                            <p className="text-sm font-medium text-emerald-700">Déjà Versé</p>
                                            <p className="font-bold text-lg text-emerald-700">{selectedDebt.amountDue.toLocaleString()} F</p>
                                        </div>
                                        <div className="flex justify-between items-center pt-1">
                                            <p className="text-sm font-black uppercase text-red-600">Reste à Payer</p>
                                            <p className="font-black text-2xl text-red-600">{(selectedDebt.amountInitial - selectedDebt.amountDue).toLocaleString()} F</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {selectedDebt.payments && selectedDebt.payments.length > 0 ? (
                                                selectedDebt.payments.map((pay: any) => (
                                                    <TableRow key={pay.id}>
                                                        <TableCell>{format(new Date(pay.paymentDate), "dd/MM/yyyy HH:mm")}</TableCell>
                                                        <TableCell className="text-muted-foreground italic">{pay.note || "-"}</TableCell>
                                                        <TableCell className="text-right font-mono font-bold text-emerald-600">+{pay.amount.toLocaleString()} F</TableCell>
                                                    </TableRow>
                                                ))
                                            ) : (
